...
function impact(data) {
    try{
        if (data.periodType === 'days') {
           ...
           const dollarInFlight = (infectionsByRequestedTime * (65 / 100)) * data.region.avgDailyIncomeInUSD * data.timeToElapse  
...
function severelyImpact(data) {
    try {
        if (data.periodType === 'days') {
            ...
            //computation for dollarInFlight 
            const dollarInFlight = (infectionsByRequestedTime * (65 / 100)) * data.region.avgDailyIncomeInUSD * convertToDays
...
const covid19ImpactEstimator = (data) => {
    try{
        ...
        let impacted = impact(data); 
        let severelyImpacted = severelyImpact(data);
